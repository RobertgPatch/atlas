...
describe('K1 tracker calculation', () => {
  ...
  it('uses canonical cash activity for basis and keeps legacy Section L contributions for reconciliation only', () => {
    ...
    expect(canonical.sectionL.reportedContributions).toBe('50.00')
    expect(canonical.sectionL.contributionDifference).toBe('30.00')
    expect(legacy.basis.contributions).toBe('0.00')
    expect(legacy.sectionL.reportedContributions).toBe('30.00')
    expect(legacy.sectionL.contributionDifference).toBe('30.00')
  })
  ...
  it('projects comparison cash values with canonical, absolute, zero, and null semantics', () => {
    ...
    expect(legacy.summary).toMatchObject({ capitalContributed: null, distributions: null })
  ...
  it('uses split Line 13 fields exactly once with a presence-based legacy fallback', () => {
    ...
    expect(split.calculationVersion).toBe('irs-k1-basis-v8-part-iii-source')
  })
  ...
  it('reduces basis and Section L capital for Box 18C nondeductible expenses', () => {
    ...
    expect(result.basis).not.toHaveProperty('inferredNondeductibleExpenses')
  ...
  it('never infers a missing Part III value from Section L reconciliation variances', () => {
    const result = calculateTrackerYear({
    ...
    expect(result.basis).not.toHaveProperty('inferredNondeductibleExpenses')
    expect(result.basis.nondeductibleExpenses).toBe('0.00')
    expect(result.basis.endingOutsideBasis).toBe('1685369.00')
    expect(result.sectionL.calculatedNetIncome).toBe('-56202.00')
    expect(result.bookTax.unexplainedVariance).toBe('-642.00')
    expect(result.checks.find((check) => check.key === 'section-l-net-income')).toMatchObject({ status: 'WARNING', blocking: false })
    expect(result.checks.find((check) => check.key === 'section-l-ending')).toMatchObject({ status: 'WARNING', blocking: false })
    expect(result.checks.find((check) => check.key === 'book-tax-unexplained')).toMatchObject({ status: 'WARNING', blocking: false })
    expect(result.summary.status).toBe('RECONCILED')
  })
  ...
  it('keeps Section L variances informational and non-blocking', () => {
    const base = { opening_outside_basis: '100.00', box_1_ordinary_income_loss: '0.00', section_l_beginning_capital: '100.00', section_l_current_year_net_income_loss: '0.00', section_l_ending_capital: '101.00', book_capital_account: '100.00' }
    ...
    expect(moreThanDollar.checks.find((check) => check.key === 'section-l-ending')).toMatchObject({ status: 'WARNING', blocking: false })
    expect(moreThanDollar.summary.status).toBe('RECONCILED')
  })
  ...
  it('matches the CPA workbook ending-basis regression values and corrects its inception-year net-income defect', async () => {
    ...
    for (const [index, importedYear] of importedYears.entries()) {
      ...
      expect(result.calculationVersion).toBe('irs-k1-basis-v8-part-iii-source')
