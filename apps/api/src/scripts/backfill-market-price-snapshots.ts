...
export const parseBackfillRange = (
  ...
  if (span > MAX_BACKFILL_DAYS) {
    throw new Error(
      `Backfills are limited to ${MAX_BACKFILL_DAYS} calendar days.`,
    );
  }
  return { from, to };
};
...
export const runBackfill = async (
  ...
  if (!config.databaseUrl) {
    throw new Error(
      "DATABASE_URL is required because backfill snapshots must be durable.",
    );
  }
  ...
  await runCostWorkload(
    {
      ...
      leaseTtlSeconds: Math.ceil(
        config.abuseProtection.timeouts.backfillMs / 1_000,
      ),
    },
    async () => {
      ...
      for (const tradingDate of dates) {
        try {
          const result =
            await marketDataService.refreshClosingPrices(tradingDate);
      ...
      if (deferredToday) {
        console.info(
          JSON.stringify({
            ...
            reason:
              "The official close is not settled until 4:20 p.m. Eastern.",
          }),
        );
      }
      console.info(
      ...
      if (failed > 0) throw new Error(`${failed} backfill date(s) failed.`);
    },
  );
};
