...
describe('Dated cash activity', () => {
  it('starts with one row and adds another cash activity row', async () => {
    ...
    render(<DatedCashFlowPanel events={[]} canEdit pending={false} onCreate={onCreate} onDelete={vi.fn()} />)
    expect(screen.getByText(/These entries do not change K-1 document fields/i)).toBeInTheDocument()
    fireEvent.click(screen.getByRole('button', { name: 'Cash Activity' }))

    const dialog = screen.getByRole('dialog', { name: 'Cash Activity' })
    ...
    expect(screen.getByLabelText('Cash activity date row 1')).not.toHaveAttribute('min')
    expect(screen.getByLabelText('Cash activity date row 1')).not.toHaveAttribute('max')
    ...
    fireEvent.change(screen.getByLabelText('Cash activity date row 1'), { target: { value: '2021-03-15' } })
    ...
    fireEvent.change(screen.getByLabelText('Cash activity date row 2'), { target: { value: '2026-10-15' } })
    ...
    await waitFor(() => expect(onCreate).toHaveBeenCalledWith([
      { kind: 'CAPITAL_CALL', activityDate: '2021-03-15', amount: '125000.00', note: null },
      { kind: 'RECALLABLE_DISTRIBUTION', activityDate: '2026-10-15', amount: '10000.00', note: null },
    ]))
  ...
  it('shows all-date totals and confirms deletion in the application modal', async () => {
    ...
    render(<DatedCashFlowPanel events={[event]} canEdit pending={false} onCreate={vi.fn()} onDelete={onDelete} />)
    expect(screen.getAllByText('$25,000.00').length).toBeGreaterThan(0)
    fireEvent.click(screen.getByRole('button', { name: /Delete non-recallable distribution/ }))
    expect(screen.getByRole('dialog', { name: 'Delete this non-recallable distribution?' })).toBeTruthy()
