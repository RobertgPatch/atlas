...
const percent = (value: string | null, status: string) => value == null ? availability(status) : `${(Number(value) * 100).toFixed(1)}%`

export function PerformanceMetricStrip({ summary }: { summary: PartnershipTrackerSummary }) {
  const status = summary.performanceStatus ?? { dpi: 'MISSING_CONTRIBUTIONS', tvpi: 'MISSING_CONTRIBUTIONS', irr: 'MISSING_CONTRIBUTIONS' }
  const metrics = [
