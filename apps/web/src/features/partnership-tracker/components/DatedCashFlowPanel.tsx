        ...
        <DialogPanel transition className="w-full overflow-hidden rounded-t-xl border border-gray-200 bg-white shadow-2xl transition duration-200 data-[closed]:translate-y-3 data-[closed]:opacity-0 motion-reduce:transform-none motion-reduce:transition-none sm:max-w-3xl sm:rounded-xl">
          ...
          <form onSubmit={save}>
            <div className="space-y-4 px-5 py-5 sm:px-6">
              ...
              <div className="max-h-[45vh] space-y-3 overflow-y-auto pr-1">{draftRows.map((row, index) => <div key={row.id} className="relative grid gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.2fr)_2.75rem] sm:items-end">
                <span className="absolute -left-2 -top-2 grid h-6 min-w-6 place-items-center rounded-full bg-gray-950 px-1 text-[0.65rem] font-bold text-white" aria-hidden="true">{index + 1}</span>
