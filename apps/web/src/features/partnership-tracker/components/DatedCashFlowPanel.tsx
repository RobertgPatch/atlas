...
const activityOptions: Array<{ kind: K1TrackerCashFlowKind; label: string }> = [
  ...
  { kind: 'DISTRIBUTION', label: 'Non-Recallable Distribution' },
...
export function DatedCashFlowPanel({ events, canEdit, pending, onCreate, onDelete }: {
  ...
  const newDraftRow = (): DraftCashFlowRow => ({ id: nextRowId.current++, activityDate: new Date().toISOString().slice(0, 10), amount: '', kind: 'CAPITAL_CALL' })
  const openDialog = () => {
    nextRowId.current = 1
  ...
  const save = async (event: React.FormEvent) => {
    ...
    for (const [index, row] of draftRows.entries()) {
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(row.activityDate)) {
        setFormError(`Row ${index + 1}: enter a valid activity date.`)
        return
      }
  ...
  return <>
    <section aria-labelledby="cash-activity-title" className="border border-gray-200 bg-white">
      <div className="flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 px-5 py-4">
        <div><p className="text-xs font-bold uppercase tracking-[0.14em] text-jackson-hover">Dated cash activity</p><h3 id="cash-activity-title" className="mt-1 text-lg font-semibold text-gray-950">Cash activity</h3><p className="mt-1 max-w-2xl text-sm text-gray-600">Record exact-dated capital calls and distributions for partnership performance. These entries do not change K-1 document fields. Recallable distributions also restore commitment.</p></div>
        {canEdit && <button type="button" onClick={openDialog} disabled={pending} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-gray-950 px-4 text-sm font-semibold text-white transition-colors hover:bg-gray-800 disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-jackson-gold focus-visible:ring-offset-2"><Plus className="h-4 w-4" aria-hidden="true" />Cash Activity</button>}
      </div>
      <div className="grid border-b border-gray-200 sm:grid-cols-3 sm:divide-x sm:divide-gray-200">
        <div className="px-5 py-3"><p className="text-xs uppercase tracking-wide text-gray-500">Total capital calls</p><p className="mt-1 font-serif text-xl font-semibold tabular-nums text-gray-950">{money(totals.CAPITAL_CALL)}</p></div>
        <div className="border-t border-gray-200 px-5 py-3 sm:border-t-0"><p className="text-xs uppercase tracking-wide text-gray-500">Total distributions</p><p className="mt-1 font-serif text-xl font-semibold tabular-nums text-gray-950">{money(distributionTotal)}</p></div>
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[44rem] text-left text-sm"><thead className="bg-gray-50 text-[0.68rem] font-bold uppercase tracking-[0.12em] text-gray-500"><tr><th className="px-5 py-3">Activity</th><th className="px-3 py-3">Date</th><th className="px-3 py-3 text-right">Amount</th><th className="px-3 py-3">Note</th><th aria-label="Actions" className="w-14 px-3 py-3" /></tr></thead><tbody className="divide-y divide-gray-100">
        ...
        {!events.length && <tr><td colSpan={5} className="px-5 py-8 text-center text-sm text-gray-500">No cash activity yet.</td></tr>}
      </tbody></table></div>
      ...
      <div className="fixed inset-0 flex items-end justify-center overflow-y-auto sm:items-center sm:p-4">
        <DialogPanel transition className="w-full overflow-hidden rounded-t-xl border border-gray-200 bg-white shadow-2xl transition duration-200 data-[closed]:translate-y-3 data-[closed]:opacity-0 motion-reduce:transform-none motion-reduce:transition-none sm:max-w-3xl sm:rounded-xl">
          ...
          <div className="flex items-start justify-between gap-4 border-b border-gray-200 px-5 py-5 sm:px-6">
            <div><p className="text-[0.68rem] font-bold uppercase tracking-[0.16em] text-jackson-hover">All-date cash ledger</p><DialogTitle className="mt-1 font-serif text-xl font-semibold text-gray-950">Cash Activity</DialogTitle><p className="mt-1 text-sm text-gray-600">Enter one or more cash activities from any year. Every row is saved together.</p></div>
            <button type="button" aria-label="Close cash activity" onClick={closeDialog} disabled={busy} className="grid min-h-11 min-w-11 place-items-center rounded-md text-gray-500 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-jackson-gold disabled:opacity-40"><X className="h-5 w-5" aria-hidden="true" /></button>
          </div>
          <form onSubmit={save}>
            <div className="space-y-4 px-5 py-5 sm:px-6">
              ...
              <div data-testid="cash-activity-draft-list" className="max-h-[45vh] space-y-4 overflow-y-auto pb-1 pl-3 pr-1 pt-3 [scrollbar-gutter:stable]">{draftRows.map((row, index) => <div key={row.id} className="relative grid gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.2fr)_2.75rem] sm:items-end">
                ...
                <label className="block text-xs font-bold uppercase tracking-wide text-gray-500 sm:text-transparent">Date<input aria-label={`Cash activity date row ${index + 1}`} type="date" required value={row.activityDate} onChange={(event) => updateDraftRow(row.id, { activityDate: event.target.value })} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 bg-white px-3 text-sm font-normal tracking-normal text-gray-950 outline-none focus:border-jackson-gold focus:ring-2 focus:ring-jackson-gold/30" /></label>
