  ...
  return <>
    <section aria-labelledby="cash-activity-title" className={workspace ? 'overflow-hidden rounded-lg border border-slate-300 bg-white shadow-sm' : 'border border-gray-200 bg-white'}>
      <div className={workspace ? 'flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-3' : 'flex flex-wrap items-start justify-between gap-4 border-b border-gray-200 px-5 py-4'}>
        <div><p className={workspace ? 'text-[0.65rem] font-semibold uppercase tracking-[0.12em] text-primary' : 'text-xs font-bold uppercase tracking-[0.14em] text-primary'}>Dated cash activity</p><h3 id="cash-activity-title" className={workspace ? 'mt-0.5 text-sm font-semibold text-slate-950' : 'mt-1 text-lg font-semibold text-gray-950'}>Net cash activity</h3><p className={workspace ? 'mt-0.5 max-w-2xl text-xs text-slate-600' : 'mt-1 max-w-2xl text-sm text-gray-600'}>Record exact-dated capital calls and distributions for annual K-1 totals and XIRR. Recallable distributions also restore commitment.</p></div>
        {canEdit && <button type="button" onClick={openDialog} disabled={pending} className={workspace ? 'inline-flex min-h-9 items-center gap-2 rounded-md border border-slate-300 bg-white px-3 text-xs font-semibold text-slate-700 shadow-sm hover:bg-slate-100 disabled:opacity-40' : 'inline-flex min-h-11 items-center gap-2 rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground transition-colors hover:bg-primary-hover disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2'}><Plus className="h-4 w-4" aria-hidden="true" />{workspace ? 'Add activity' : 'Net Cash Activity'}</button>}
      </div>
      ...
      <div className="fixed inset-0 flex items-end justify-center overflow-y-auto sm:items-center sm:p-4">
        <DialogPanel transition className="w-full overflow-hidden rounded-t-xl border border-gray-200 bg-white shadow-2xl transition duration-200 data-[closed]:translate-y-3 data-[closed]:opacity-0 motion-reduce:transform-none motion-reduce:transition-none sm:max-w-3xl sm:rounded-xl">
          ...
          <div className="flex items-start justify-between gap-4 border-b border-gray-200 px-5 py-5 sm:px-6">
            <div><p className="text-[0.68rem] font-bold uppercase tracking-[0.16em] text-primary">{taxYear} cash ledger</p><DialogTitle className="mt-1 font-serif text-xl font-semibold text-gray-950">Net Cash Activity</DialogTitle><p className="mt-1 text-sm text-gray-600">Enter one or more cash activities. Every row is saved together.</p></div>
            <button type="button" aria-label="Close net cash activity" onClick={closeDialog} disabled={busy} className="grid min-h-11 min-w-11 place-items-center rounded-md text-gray-500 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus disabled:opacity-40"><X className="h-5 w-5" aria-hidden="true" /></button>
          </div>
          <form onSubmit={save}>
            <div className="space-y-4 px-5 py-5 sm:px-6">
              ...
              <div data-testid="cash-activity-draft-list" className="max-h-[45vh] space-y-4 overflow-y-auto pb-1 pl-3 pr-1 pt-3 [scrollbar-gutter:stable]">{draftRows.map((row, index) => <div key={row.id} className="relative grid gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.2fr)_2.75rem] sm:items-end">
                ...
                <label className="block text-xs font-bold uppercase tracking-wide text-gray-500 sm:text-transparent">Date<input aria-label={`Cash activity date row ${index + 1}`} type="date" required min={`${taxYear}-01-01`} max={`${taxYear}-12-31`} value={row.activityDate} onChange={(event) => updateDraftRow(row.id, { activityDate: event.target.value })} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 bg-white px-3 text-sm font-normal tracking-normal text-gray-950 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
                ...
                <label className="block text-xs font-bold uppercase tracking-wide text-gray-500 sm:text-transparent">Type<select aria-label={`Cash activity type row ${index + 1}`} required value={row.kind} onChange={(event) => updateDraftRow(row.id, { kind: event.target.value as K1TrackerCashFlowKind })} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 bg-white px-3 text-sm font-normal tracking-normal text-gray-950 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30">{activityOptions.map((option) => <option key={option.kind} value={option.kind}>{option.label}</option>)}</select></label>
                <button type="button" aria-label={`Remove cash activity row ${index + 1}`} onClick={() => removeDraftRow(row.id)} disabled={draftRows.length === 1 || busy} className="grid min-h-11 min-w-11 place-items-center rounded-md border border-gray-300 bg-white text-gray-500 hover:border-red-200 hover:bg-red-50 hover:text-red-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus disabled:cursor-not-allowed disabled:opacity-30"><Trash2 className="h-4 w-4" aria-hidden="true" /></button>
              </div>)}</div>
              <button type="button" onClick={addDraftRow} disabled={busy || draftRows.length >= 50} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-dashed border-gray-400 bg-white px-4 text-sm font-semibold text-gray-800 hover:border-primary hover:bg-amber-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus disabled:opacity-40"><Plus className="h-4 w-4" aria-hidden="true" />Add another row</button>
              {draftRows.some((row) => row.kind === 'RECALLABLE_DISTRIBUTION') && <p className="border-l-4 border-primary bg-amber-50 px-3 py-2 text-sm text-amber-950">Each recallable distribution increases committed capital by the same amount. Deleting one later reverses its increase.</p>}
              {formError && <p role="alert" className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{formError}</p>}
            </div>
            <div className="flex flex-col-reverse gap-3 border-t border-gray-200 bg-gray-50 px-5 py-4 sm:flex-row sm:justify-end sm:px-6"><button type="button" onClick={closeDialog} disabled={busy} className="min-h-11 rounded-md border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-700 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus disabled:opacity-40">Cancel</button><button type="submit" disabled={busy} className="min-h-11 rounded-md bg-primary px-5 text-sm font-semibold text-primary-foreground hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2 disabled:opacity-40">{busy ? 'Saving\u2026' : `Add ${draftRows.length} activit${draftRows.length === 1 ? 'y' : 'ies'}`}</button></div>
          </form>
        </DialogPanel>
      </div>
    </Dialog>
