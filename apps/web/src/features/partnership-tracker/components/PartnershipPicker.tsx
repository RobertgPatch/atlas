...
export function PartnershipPicker({ items, selectedId, search, loading, error, canEdit, onSearch, onSelect, onAdd }: {
  ...
  return <aside className="rounded-xl border border-gray-200 bg-white shadow-sm" aria-label="Partnership directory">
    <div className="border-b border-gray-200 p-4">
      <div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-gray-950">Partnerships</h2>{canEdit && <button type="button" onClick={onAdd} className="inline-flex items-center gap-1 rounded-md bg-atlas-gold px-2.5 py-1.5 text-xs font-semibold text-white hover:bg-atlas-hover"><Plus className="h-3.5 w-3.5" />Add</button>}</div>
      <label className="relative mt-3 block"><span className="sr-only">Search partnerships</span><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-gray-400" /><input value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search partnerships or entities" className="w-full rounded-lg border border-gray-300 py-2 pl-9 pr-3 text-sm focus:border-atlas-gold focus:outline-none focus:ring-2 focus:ring-atlas-gold/20" /></label>
    </div>
