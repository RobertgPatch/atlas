  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/60 p-4" role="presentation">
    <div role="dialog" aria-modal="true" aria-labelledby="add-partnership-title" className="flex max-h-[calc(100vh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-lg bg-white shadow-2xl">
      <div className="shrink-0 border-b border-gray-200 px-6 py-5">
        <p className="text-[0.68rem] font-bold uppercase tracking-[0.16em] text-primary">Partnership workspace</p>
      ...
      <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        <fieldset>
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            <label className={`cursor-pointer rounded-md border p-3 transition-colors ${mode === 'new' ? 'border-primary bg-primary/10 ring-1 ring-focus' : 'border-gray-300 hover:border-gray-400'}`}>
              <span className="flex items-start gap-3"><input type="radio" name="creation-mode" value="new" checked={mode === 'new'} onChange={() => chooseMode('new')} className="mt-1 h-4 w-4 border-gray-300 text-primary focus:ring-focus" /><span><span className="block text-sm font-semibold text-gray-950">New partnership</span><span className="mt-1 block text-xs leading-4 text-gray-500">Start a new aggregate and owner record.</span></span></span>
            </label>
            <label className={`cursor-pointer rounded-md border p-3 transition-colors ${mode === 'existing' ? 'border-primary bg-primary/10 ring-1 ring-focus' : 'border-gray-300 hover:border-gray-400'}`}>
        ...
        {mode === 'existing' ? <label className="block text-sm font-medium text-gray-800">Existing partnership
          ...
          {selectedPartnership && <span className="mt-2 block border-l-2 border-primary pl-3 text-xs leading-5 text-gray-500">The new record will appear under <strong className="text-gray-700">{selectedPartnership.partnership.name}</strong> on All Partnerships. K-1s remain owner-specific unless you copy selected years below.</span>}
        </label> : <>
          <label className="block text-sm font-medium text-gray-800">Partnership name<input ref={nameRef} value={name} required maxLength={120} onChange={(event) => setName(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
          <label className="block text-sm font-medium text-gray-800">Partnership type<select value={type} onChange={(event) => setType(event.target.value as PartnershipType)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 bg-white px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30">{PARTNERSHIP_TYPES.map((option) => <option key={option}>{option}</option>)}</select></label>
        </>}

        <label className="block text-sm font-medium text-gray-800">Owner<select value={entityId} required onChange={(event) => setEntityId(event.target.value)} disabled={entities.isLoading || !availableOwners.length || (mode === 'existing' && !selectedPartnership)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 bg-white px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30"><option value="">{entities.isLoading ? 'Loading owners...' : mode === 'existing' && !selectedPartnership ? 'Select a partnership first' : 'Select an owner'}</option>{availableOwners.map((entity) => <option key={entity.id} value={entity.id}>{entity.name}</option>)}</select></label>
        ...
        <section aria-labelledby="copy-k1-years-heading" className="relative overflow-hidden rounded-md border border-gray-200 bg-gray-50/70 p-4 pl-5">
          ...
          <label className="flex cursor-pointer items-start gap-3">
            <input
              ...
              className="mt-1 h-4 w-4 rounded border-gray-300 text-primary focus:ring-focus"
            />
          ...
          {copyK1Years ? <div className="mt-4 space-y-4 border-t border-gray-200 pt-4">
            <label className="block text-sm font-medium text-gray-800">Source partnership
              <select value={copySourcePartnershipId} required onChange={(event) => chooseCopySource(event.target.value)} disabled={partnerships.isLoading} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 bg-white px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30">
            ...
            {copySource.isLoading ? <p aria-live="polite" className="text-sm text-gray-600">Loading available K-1 years…</p> : copySource.isError ? <p role="alert" className="text-sm text-red-700">The source partnership's K-1 years could not be loaded.</p> : copySourcePartnershipId && !availableCopyYears.length ? <p className="text-sm text-amber-800">This partnership does not have any K-1 entry years to copy.</p> : availableCopyYears.length ? <fieldset>
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="flex gap-3 text-xs font-semibold"><button type="button" onClick={() => setExcludedCopyYears([])} className="text-primary underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">Select all</button><button type="button" onClick={() => setExcludedCopyYears(availableCopyYears.map((year) => year.taxYear))} className="text-gray-600 underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">Clear</button></span>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
                {availableCopyYears.map((year) => <label key={year.taxYear} className={`flex min-h-11 cursor-pointer items-center justify-between rounded-md border px-3 py-2 text-sm font-semibold ${selectedCopyYears.includes(year.taxYear) ? 'border-primary bg-white text-gray-950 ring-1 ring-focus/40' : 'border-gray-300 bg-white/60 text-gray-600'}`}><span>{year.taxYear}</span><input type="checkbox" checked={selectedCopyYears.includes(year.taxYear)} onChange={() => toggleCopyYear(year.taxYear)} className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-focus" /></label>)}
              </div>
        ...
        {mode === 'new' && <>
          <section aria-labelledby="partnership-profile-heading" className="border-t border-gray-200 pt-5">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <label className="block text-sm font-medium text-gray-800">Inception date<input type="date" max={new Date().toISOString().slice(0, 10)} value={inceptionDate} onChange={(event) => setInceptionDate(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
              <label className="block text-sm font-medium text-gray-800">EIN <span className="font-normal text-gray-500">(masked on overview)</span><input inputMode="numeric" autoComplete="off" value={ein} onChange={(event) => setEin(formatEinInput(event.target.value))} placeholder="12-3456789" pattern="[0-9]{2}-[0-9]{7}" className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /><span className="mt-1 block text-xs font-normal text-gray-500">Formatted automatically as 12-3456789.</span></label>
              <label className="block text-sm font-medium text-gray-800 sm:col-span-2">Fund manager<input value={fundManager} maxLength={200} onChange={(event) => setFundManager(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
              <label className="block text-sm font-medium text-gray-800 sm:col-span-2">Address<input value={addressLine1} maxLength={200} onChange={(event) => setAddressLine1(event.target.value)} placeholder="Street address" className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
              <label className="block text-sm font-medium text-gray-800 sm:col-span-2"><span className="sr-only">Address line 2</span><input value={addressLine2} maxLength={200} onChange={(event) => setAddressLine2(event.target.value)} placeholder="Suite, unit, or building (optional)" className="min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
              <label className="block text-sm font-medium text-gray-800">City<input value={addressCity} maxLength={120} onChange={(event) => setAddressCity(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
              <label className="block text-sm font-medium text-gray-800">State / region<input value={addressRegion} maxLength={120} onChange={(event) => setAddressRegion(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
              <label className="block text-sm font-medium text-gray-800">Postal code<input value={addressPostalCode} maxLength={30} onChange={(event) => setAddressPostalCode(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
              <label className="block text-sm font-medium text-gray-800">Country<input value={addressCountry} maxLength={120} onChange={(event) => setAddressCountry(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
            </div>
          </section>
          <section aria-labelledby="entry-valuation-heading" className="border-t border-gray-200 pt-5"><h3 id="entry-valuation-heading" className="font-serif text-lg font-semibold text-gray-950">Entry valuation</h3><p className="mt-1 text-xs text-gray-500">Optional. This becomes the first dated NAV entry.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="block text-sm font-medium text-gray-800">Initial valuation<CurrencyInput value={initialValuationAmount} onChange={setInitialValuationAmount} allowNegative={false} placeholder="$0.00" /></label><label className="block text-sm font-medium text-gray-800">Valuation date<input type="date" value={initialValuationDate} onChange={(event) => setInitialValuationDate(event.target.value)} className="mt-1 min-h-11 w-full rounded-md border border-gray-300 px-3 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label></div></section>
        </>}
        <label className="block text-sm font-medium text-gray-800">Notes <span className="font-normal text-gray-500">(optional, owner-specific)</span><textarea value={notes} maxLength={10_000} rows={3} onChange={(event) => setNotes(event.target.value)} className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" /></label>
        </div>
        <div className="shrink-0 border-t border-gray-200 bg-white px-6 py-4 shadow-[0_-8px_20px_-16px_rgba(15,23,42,0.45)]" data-testid="add-partnership-dialog-footer">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={onClose} className="min-h-11 rounded-md border border-gray-300 px-4 text-sm font-semibold text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">Cancel</button><button type="submit" disabled={actions.createPartnership.isPending || !availableOwners.length || copySelectionInvalid} className="min-h-11 rounded-md bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary-hover disabled:cursor-not-allowed disabled:bg-disabled-background disabled:text-disabled-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2">{actions.createPartnership.isPending ? 'Creating…' : mode === 'existing' ? 'Add owner record' : 'Create partnership'}</button></div>
        </div>
      </form>
    </div>
  </div>
