...
export function EditPartnershipDialog({ summary, onClose }: { summary: PartnershipTrackerSummary; onClose: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      await actions.updatePartnership.mutateAsync({
        id: partnership.id,
        body: { entityId, name: name.trim(), partnershipType: type, status, notes: notes.trim() || null, expectedUpdatedAt: partnership.updatedAt },
      })
      onClose()
    } catch (caught) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><div role="dialog" aria-modal="true" aria-labelledby="edit-partnership-title" className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl"><h2 id="edit-partnership-title" className="text-lg font-semibold text-gray-950">Edit partnership</h2><form onSubmit={submit} className="mt-5 space-y-4">
