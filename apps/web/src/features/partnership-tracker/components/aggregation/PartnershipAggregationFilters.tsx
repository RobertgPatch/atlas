  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white">
      <div className="flex items-start justify-between border-b border-gray-200 px-4 py-4">
        <div>
          <p className="text-[0.68rem] font-bold uppercase tracking-[0.16em] text-primary">Portfolio index</p>
          <h2 className="mt-1 font-serif text-xl font-semibold text-gray-950">Refine the book</h2>
        </div>
        {onDone && <button type="button" aria-label="Close filters" onClick={onDone} className="grid min-h-11 min-w-11 place-items-center rounded-md text-gray-500 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus"><X className="h-5 w-5" /></button>}
      </div>
      <div data-testid={onDone ? 'aggregation-filter-scroll-mobile' : 'aggregation-filter-scroll'} className="min-h-0 flex-1 overflow-y-auto px-4 py-5">
        ...
        <div className="relative mt-2">
          ...
          <input id={onDone ? 'aggregation-search-mobile' : 'aggregation-search'} type="search" value={searchValue} onChange={(event) => onSearchChange(event.target.value)} placeholder="Name or owner" className="min-h-11 w-full rounded-md border border-gray-300 bg-gray-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-focus focus:ring-2 focus:ring-focus/30" />
        </div>
      ...
      <div className="flex gap-3 border-t border-gray-200 p-4">
        <button type="button" onClick={onClear} disabled={activeCount === 0} className="min-h-11 flex-1 rounded-md border border-gray-300 px-3 text-sm font-semibold text-gray-700 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">Clear all{activeCount ? ` (${activeCount})` : ''}</button>
        {onDone && <button type="button" onClick={onDone} className="min-h-11 flex-1 rounded-md bg-primary px-3 text-sm font-semibold text-primary-foreground hover:bg-primary-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus focus-visible:ring-offset-2">Show results</button>}
      </div>
    </div>
  )
}
...
export function PartnershipAggregationFilters(props: PartnershipAggregationFiltersProps) {
  ...
  return (
    <div className="w-full lg:w-[17rem] lg:shrink-0">
      ...
      <button type="button" onClick={() => setDrawerOpen(true)} className="inline-flex min-h-11 items-center gap-2 rounded-md border border-gray-300 bg-white px-4 text-sm font-semibold text-gray-800 shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus lg:hidden">
