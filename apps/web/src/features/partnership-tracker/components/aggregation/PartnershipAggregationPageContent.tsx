...
export function PartnershipAggregationPageContent({ canEdit }: { canEdit: boolean }) {
  ...
  return (
    <div className="relative border-l-4 border-primary pl-3 sm:pl-5">
      <PageHeader
        title="Partnership aggregation"
        subtitle="A filterable, exact-value ledger of every partnership in your permitted portfolio."
        actions={<><PartnershipViewSwitcher view="aggregation" />{canEdit && <Button type="button" onClick={() => setAdding(true)}><Plus className="h-4 w-4" /> Add partnership</Button>}</>}
      />
      ...
      <div className="flex flex-col items-start gap-5 lg:flex-row">
        ...
        <main className="min-w-0 flex-1" aria-label="Partnership aggregation results">
          <div className="mb-3 flex min-h-11 flex-wrap items-center gap-2">
            ...
            {query.search && <button type="button" onClick={() => { setSearchDraft(null); setQuery({ ...query, search: undefined, page: 1 }) }} className="inline-flex min-h-9 items-center gap-1 rounded-full border border-gray-300 bg-white px-3 text-xs font-semibold text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">Search: “{query.search}” <X className="h-3 w-3" /></button>}
            {activeFilters.map((filter) => <button key={`${filter.key}-${filter.value}`} type="button" onClick={() => toggleFilter(filter.key, filter.value)} className="inline-flex min-h-9 items-center gap-1 rounded-full border border-gray-300 bg-white px-3 text-xs font-semibold text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">{filter.label} <X className="h-3 w-3" /></button>)}
            {activeCount > 0 && <button type="button" onClick={clearAll} className="min-h-9 px-2 text-xs font-bold text-gray-600 underline decoration-primary underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">Clear all</button>}
          ...
          {aggregation.isLoading ? (
            ...
            <section className="border border-dashed border-gray-400 bg-white px-6 py-16 text-center"><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Portfolio index</p><h2 className="mt-2 font-serif text-2xl font-semibold text-gray-950">No partnerships in your scope</h2><p className="mx-auto mt-3 max-w-lg text-sm text-gray-600">{canEdit ? 'Add the first partnership to begin building the portfolio ledger.' : 'No partnership records are available for your permitted owners.'}</p>{canEdit && <Button type="button" onClick={() => setAdding(true)} className="mt-6">Add partnership</Button>}</section>
          ) : aggregation.data ? (
            <div className="space-y-5">
              ...
              {aggregation.data.pageInfo.totalItems === 0 ? (
                <section className="border border-dashed border-gray-400 bg-white px-6 py-14 text-center"><h2 className="font-serif text-2xl font-semibold text-gray-950">No partnerships match these filters</h2><p className="mt-3 text-sm text-gray-600">The portfolio is still intact. Clear one or more filters to widen the result set.</p><button type="button" onClick={clearAll} className="mt-5 min-h-11 rounded-md border border-gray-400 bg-white px-4 text-sm font-bold text-gray-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus">Clear all filters</button></section>
