...
export function PartnershipAggregationPageContent({ canEdit }: { canEdit: boolean }) {
  ...
  return (
    <div className="relative border-l-4 border-atlas-gold pl-3 sm:pl-5">
      <PageHeader
        title="Partnership aggregation"
        subtitle="A filterable, exact-value ledger of every partnership in your permitted portfolio."
        actions={<><PartnershipViewSwitcher view="aggregation" />{canEdit && <button type="button" onClick={() => setAdding(true)} className="inline-flex min-h-11 items-center gap-2 rounded-md bg-atlas-gold px-4 text-sm font-bold text-gray-950 shadow-sm hover:bg-atlas-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold focus-visible:ring-offset-2"><Plus className="h-4 w-4" /> Add partnership</button>}</>}
      />
      ...
      <div className="flex flex-col items-start gap-5 lg:flex-row">
        ...
        <main className="min-w-0 flex-1" aria-label="Partnership aggregation results">
          <div className="mb-3 flex min-h-11 flex-wrap items-center gap-2">
            ...
            {query.search && <button type="button" onClick={() => { setSearchDraft(null); setQuery({ ...query, search: undefined, page: 1 }) }} className="inline-flex min-h-9 items-center gap-1 rounded-full border border-gray-300 bg-white px-3 text-xs font-semibold text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold">Search: “{query.search}” <X className="h-3 w-3" /></button>}
            {activeFilters.map((filter) => <button key={`${filter.key}-${filter.value}`} type="button" onClick={() => toggleFilter(filter.key, filter.value)} className="inline-flex min-h-9 items-center gap-1 rounded-full border border-gray-300 bg-white px-3 text-xs font-semibold text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold">{filter.label} <X className="h-3 w-3" /></button>)}
            {activeCount > 0 && <button type="button" onClick={clearAll} className="min-h-9 px-2 text-xs font-bold text-gray-600 underline decoration-atlas-gold underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold">Clear all</button>}
          ...
          {aggregation.isLoading ? (
            ...
            <section className="border border-dashed border-gray-400 bg-white px-6 py-16 text-center"><p className="text-xs font-bold uppercase tracking-[0.16em] text-atlas-hover">Portfolio index</p><h2 className="mt-2 font-serif text-2xl font-semibold text-gray-950">No partnerships in your scope</h2><p className="mx-auto mt-3 max-w-lg text-sm text-gray-600">{canEdit ? 'Add the first partnership to begin building the portfolio ledger.' : 'No partnership records are available for your permitted owners.'}</p>{canEdit && <button type="button" onClick={() => setAdding(true)} className="mt-6 min-h-11 rounded-md bg-gray-950 px-5 text-sm font-bold text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold focus-visible:ring-offset-2">Add partnership</button>}</section>
          ) : aggregation.data ? (
