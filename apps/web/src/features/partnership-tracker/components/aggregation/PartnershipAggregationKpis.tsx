...
import { formatExactMoney, formatLedgerDate, formatMultiple, humanizeCode } from './aggregationFormatters'
...
export function PartnershipAggregationKpis({ rollup }: { rollup: PartnershipPortfolioRollup }) {
  const metrics = [
    { label: 'Committed capital', value: formatExactMoney(rollup.committedCapital.amount), detail: coverage(rollup.committedCapital.knownCount, rollup.committedCapital.totalCount) },
    { label: 'Paid-in capital', value: formatExactMoney(rollup.paidInCapital.amount), detail: coverage(rollup.paidInCapital.knownCount, rollup.paidInCapital.totalCount) },
    { label: 'Distributions', value: formatExactMoney(rollup.distributions.amount), detail: coverage(rollup.distributions.knownCount, rollup.distributions.totalCount) },
    { label: 'Latest NAV', value: formatExactMoney(rollup.latestNav.amount), detail: rollup.navValuationRange.earliest && rollup.navValuationRange.latest ? `${formatLedgerDate(rollup.navValuationRange.earliest)} – ${formatLedgerDate(rollup.navValuationRange.latest)}` : 'No NAV dates in scope' },
    { label: 'Unfunded', value: formatExactMoney(rollup.unfundedCommitment.amount), detail: coverage(rollup.unfundedCommitment.knownCount, rollup.unfundedCommitment.totalCount) },
