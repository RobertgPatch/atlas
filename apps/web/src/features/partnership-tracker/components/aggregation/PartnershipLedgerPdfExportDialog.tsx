      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-2xl overflow-hidden rounded-lg bg-white shadow-2xl">
          <div className="flex items-start justify-between border-b border-gray-200 px-5 py-4">
            ...
            <button type="button" aria-label="Close PDF export" onClick={onClose} className="grid min-h-11 min-w-11 place-items-center rounded-md text-gray-500 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold"><X className="h-5 w-5" /></button>
          </div>
          <div className="max-h-[60vh] overflow-y-auto px-5 py-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h3 className="text-sm font-bold uppercase tracking-[0.12em] text-gray-700">Columns</h3>
              <div className="flex gap-3 text-sm font-semibold">
                <button type="button" onClick={() => setSelectedColumnIds(allColumnIds)} className="min-h-9 text-gray-700 underline decoration-atlas-gold underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold">All columns</button>
                <button type="button" onClick={() => setSelectedColumnIds([])} className="min-h-9 text-gray-700 underline decoration-atlas-gold underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold">Clear</button>
              </div>
            </div>
            <fieldset className="mt-4 grid gap-2 sm:grid-cols-2">
              ...
              {partnershipLedgerColumns.map((column) => (
                <label key={column.id} className="flex min-h-11 cursor-pointer items-center gap-3 border-b border-gray-100 px-2 text-sm text-gray-800 hover:bg-gray-50">
                  <input type="checkbox" checked={selectedColumnIds.includes(column.id)} onChange={() => toggleColumn(column.id)} className="h-4 w-4 rounded border-gray-300 text-atlas-gold focus:ring-atlas-gold" />
                  {column.label}
                </label>
              ))}
            </fieldset>
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-gray-200 px-5 py-4 sm:flex-row sm:justify-end">
            <button type="button" onClick={onClose} className="min-h-11 rounded-md border border-gray-300 px-4 text-sm font-semibold text-gray-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold">Cancel</button>
            <button type="button" onClick={exportPdf} disabled={!rows.length || !selectedColumns.length} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-gray-950 px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-atlas-gold focus-visible:ring-offset-2"><FileDown className="h-4 w-4" /> Export PDF</button>
          </div>
        </DialogPanel>
      </div>
    </Dialog>
