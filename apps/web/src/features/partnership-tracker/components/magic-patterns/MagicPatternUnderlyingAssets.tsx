...
function formatValuationDate(value: string | null | undefined) {
        ...
        <fieldset>
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">
            {ASSET_CATEGORY_DEFINITIONS.map((option) => {
              ...
              return <button key={option.id} type="button" aria-pressed={assetCategory === option.id} onClick={() => changeCategory(option.id)} className={`rounded-md border p-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus ${assetCategory === option.id ? CATEGORY_TONES[option.id] : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50'}`}><Icon className="h-4 w-4" /><span className="mt-2 block text-xs font-semibold leading-4">{option.label}</span></button>
            })}
          </div>
...
export function MagicPatternUnderlyingAssets({
  ...
  const rows: PartnershipAssetRow[] = assets.data?.rows ?? []
