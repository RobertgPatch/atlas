        ...
        <section aria-label="Select counterparty">
          ...
          <div className="mt-3 max-h-72 space-y-2 overflow-y-auto">
            {visibleParties.map((candidate) => (
              <button key={candidate.id} type="button" onClick={() => setPartyId(candidate.id)} className={`w-full rounded-md border px-3 py-2.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus ${partyId === candidate.id ? 'border-primary bg-primary-subtle' : 'border-slate-200 bg-white hover:bg-slate-50'}`}>
      ...
      {step === 2 ? (
        <section aria-label="Relationship type" className="grid gap-3 sm:grid-cols-2">
          {([
            ...
            <button key={value} type="button" aria-pressed={kind === value} onClick={() => setKind(value)} className={`rounded-md border p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus ${kind === value ? 'border-primary bg-primary-subtle' : 'border-slate-300 bg-white hover:bg-slate-50'}`}>
              ...
              <div className="mt-2 space-y-2 rounded-md border border-slate-200 bg-slate-50 p-3">
                {eligibleMaps.map((map) => (
                  <label key={map.id} className="flex items-center gap-2 text-sm text-slate-800">
                    <input type="checkbox" checked={estateMapIds.includes(map.id)} onChange={(event) => setEstateMapIds((current) => event.target.checked ? [...current, map.id] : current.filter((id) => id !== map.id))} className="h-4 w-4 rounded border-slate-300 accent-primary focus:ring-focus" />
                    {map.name}
                  </label>
                ))}
              </div>
          ...
          <div className="mt-4 space-y-2">
            {eligibleMaps.map((map) => (
              <label key={map.id} className="flex items-center gap-3 rounded-md border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-medium text-slate-800">
                <input type="checkbox" checked={selectedIds.includes(map.id)} onChange={(event) => setSelectedIds((current) => event.target.checked ? [...current, map.id] : current.filter((id) => id !== map.id))} className="h-4 w-4 rounded border-slate-300 accent-primary focus:ring-focus" />
                {map.name}
              </label>
            ))}
          </div>
