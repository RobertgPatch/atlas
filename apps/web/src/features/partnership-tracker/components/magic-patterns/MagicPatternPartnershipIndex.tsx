...
const formatPaidIn = (value: string | null | undefined) => {
  const formatted = formatMoney(value)
  if (formatted == null) return null
  return Number(value) === 0 ? formatted : `(${formatted})`
}
const paidInTone = (value: string | null | undefined) => Number(value) === 0 ? 'text-slate-900' : 'text-red-800'
...
export function MagicPatternPartnershipActivitySummary({ rollup }: { rollup: PartnershipPortfolioRollup }) {
  ...
  const capitalRows = [
    {
      ...
      value: formatMoney(rollup.committedCapital.amount) ?? 'Not available',
      basis: coverage(rollup.committedCapital.knownCount, rollup.committedCapital.totalCount),
    },
    {
      label: 'Paid in to date',
      value: formatMoney(rollup.paidInCapital.amount) ?? 'Not available',
      basis: coverage(rollup.paidInCapital.knownCount, rollup.paidInCapital.totalCount),
    },
    {
      label: 'Distributions received',
      value: formatMoney(rollup.distributions.amount) ?? 'Not available',
      basis: coverage(rollup.distributions.knownCount, rollup.distributions.totalCount),
    },
    {
  ...
  const performanceRows = [
    {
      ...
      basis: excluded > 0
        ? `${excluded} owner record${excluded === 1 ? '' : 's'} excluded — no valuation on file`
        : coverage(rollup.latestNav.knownCount, rollup.latestNav.totalCount),
      context: rollup.navValuationRange.latest ? formatDate(rollup.navValuationRange.latest) : undefined,
      status: rollup.latestNav.amount == null ? humanize('NOT_AVAILABLE') : 'Calculated',
      statusTone: rollup.latestNav.amount == null ? 'neutral' as const : 'calculated' as const,
    },
    {
      label: 'DPI',
      value: formatMultiple(rollup.dpi.value) ?? humanize(rollup.dpi.status),
      basis: rollup.dpi.value == null
        ? 'Non-recallable distributions divided by paid-in capital'
        : `${formatMoney(rollup.distributions.amount) ?? '—'} distributed on ${formatMoney(rollup.paidInCapital.amount) ?? '—'} paid in`,
      context: formatDate(rollup.asOfDate),
      status: rollup.dpi.value == null ? humanize(rollup.dpi.status) : 'Calculated',
      statusTone: rollup.dpi.value == null ? 'neutral' as const : 'calculated' as const,
    },
    {
      label: 'TVPI',
      value: formatMultiple(rollup.tvpi.value) ?? '— No NAV on file',
      basis: 'Distributions plus current value against capital deployed',
      context: formatDate(rollup.asOfDate),
  ...
  return (
    <MagicPatternActivitySummaryTable
      ...
      notice={stale ? <MagicStatusBadge tone="warning">Contains stale valuations</MagicStatusBadge> : undefined}
    />
  )
}

const columns = [
  ['fund', 'Fund', 235], ['owner', 'Owner', 320], ['type', 'Type', 135], ['lifecycle', 'Lifecycle', 118], ['workflow', 'K-1 workflow', 140], ['commitment', 'Commitment', 145], ['paidIn', 'Paid in', 145], ['distributions', 'Distributions', 145], ['nav', 'Latest NAV', 160], ['unfunded', 'Unfunded', 145], ['dpi', 'DPI', 90], ['tvpi', 'TVPI', 90], ['cashYield', 'Ann. COC', 105], ['actions', '', 86],
] as const
