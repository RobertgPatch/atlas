...
export function NavHistoryPanel({ partnershipId, items, canEdit }: { partnershipId: string; items: PartnershipNavEntry[]; canEdit: boolean }) {
  ...
  const remove = async (entry: PartnershipNavEntry) => {
    if (!window.confirm(`Delete the NAV value dated ${displayDate(entry.valuationDate)}?`)) return
    setError(undefined)
