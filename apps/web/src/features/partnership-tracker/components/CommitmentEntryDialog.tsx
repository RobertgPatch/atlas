...
export function CommitmentEntryDialog({ partnershipId, entry, onClose }: { partnershipId: string; entry?: PartnershipCommitmentEntry; onClose: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (entry) await actions.updateCommitment.mutateAsync({ id: partnershipId, entryId: entry.id, body: { amount, effectiveDate, note: note.trim() || null, expectedUpdatedAt: entry.updatedAt } })
      else await actions.createCommitment.mutateAsync({ id: partnershipId, body: { amount, effectiveDate, note: note.trim() || null } })
      onClose()
    } catch (caught) {
