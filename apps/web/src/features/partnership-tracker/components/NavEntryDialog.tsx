...
export function NavEntryDialog({ partnershipId, entry, onClose }: { partnershipId: string; entry?: PartnershipNavEntry; onClose: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (entry) await actions.updateNav.mutateAsync({ id: partnershipId, entryId: entry.id, body: { amount, valuationDate, note: note.trim() || null, expectedUpdatedAt: entry.updatedAt } })
      else await actions.createNav.mutateAsync({ id: partnershipId, body: { amount, valuationDate, note: note.trim() || null } })
      onClose()
    } catch (caught) {
