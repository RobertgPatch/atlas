              ...
              <Dialog.Panel className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    ...
                    <select value={assetId} onChange={(event) => setAssetId(event.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-focus ${errors.assetId ? 'border-red-500' : 'border-gray-300'}`}>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input type="date" max={today} value={valuationDate} onChange={(event) => setValuationDate(event.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-focus ${errors.valuationDate ? 'border-red-500' : 'border-gray-300'}`} />
                    ...
                    <div>
                      ...
                      <input type="number" min="0" step="0.01" value={amountUsd} onChange={(event) => setAmountUsd(event.target.value)} className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-focus ${errors.amountUsd ? 'border-red-500' : 'border-gray-300'}`} />
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <select value={source} onChange={(event) => setSource(event.target.value as AssetFmvSource)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-focus">
                    ...
                    <div>
                      ...
                      <input value={confidenceLabel} onChange={(event) => setConfidenceLabel(event.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-focus" />
                    </div>
                  </div>

                  <div>
                    <label className="mb-1 block text-sm font-medium text-text-primary">Note</label>
                    <textarea rows={3} value={note} onChange={(event) => setNote(event.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-focus" />
                  </div>
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    ...
                    <button type="submit" disabled={isPending} className="rounded-lg bg-primary px-4 py-2 text-sm text-white hover:bg-primary-hover disabled:opacity-50">
