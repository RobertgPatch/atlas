...
describe('YearRail', () => {
  it('keeps a large year history in a compact, selectable rail', () => {
    const select = vi.fn()
    render(<YearRail selectedYear={2020} onSelect={select} years={Array.from({ length: 50 }, (_, index) => ({ taxYear: 1971 + index, status: 'IMPORTED' as const, revision: 1, endingOutsideBasis: '0.00', cumulativeSuspendedLoss: '0.00', taxableExcessDistribution: '0.00', sectionLDifference: '0.00', warningCount: index === 49 ? 2 : 0, sourceConflictCount: 0 }))} />)
  ...
  it('supports nonconsecutive years and prefetches when a year receives focus', () => {
    ...
    render(<YearRail selectedYear={2021} onSelect={select} onPrefetch={prefetch} years={[2021, 2024].map((taxYear) => ({ taxYear, status: 'IMPORTED' as const, revision: 1, endingOutsideBasis: '0.00', cumulativeSuspendedLoss: '0.00', taxableExcessDistribution: '0.00', sectionLDifference: '0.00', warningCount: taxYear === 2024 ? 1 : 0, sourceConflictCount: 0 }))} />)
  ...
  it('shows workflow status for a one-year tracker and supports Enter and Space selection', () => {
    const select = vi.fn()
    render(<YearRail selectedYear={2024} onSelect={select} years={[{ taxYear: 2024, status: 'NEEDS_REVIEW', revision: 1, endingOutsideBasis: '0.00', cumulativeSuspendedLoss: '0.00', taxableExcessDistribution: '0.00', sectionLDifference: '0.00', warningCount: 0, sourceConflictCount: 0 }]} />)
