...
import { ImportWorkbookDialog } from './ImportWorkbookDialog'
...
export function K1TrackerPageContent({ canEdit }: { canEdit: boolean }) {
  ...
  return <><PageHeader title="K1 Tracker" subtitle="A partnership-first, year-by-year basis workspace with source provenance and reconciliation checks." actions={canEdit ? <div className="flex gap-2"><button type="button" onClick={() => setAddingYear(true)} className="inline-flex items-center gap-2 rounded-lg bg-atlas-gold px-4 py-2 text-sm font-medium text-white hover:bg-atlas-hover"><Plus className="h-4 w-4" />Add year</button><button type="button" onClick={() => setImporting(true)} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-50"><Upload className="h-4 w-4" />Import workbook</button></div> : null} />{message && <div role="status" className="mb-5 flex gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />{message}</div>}<div className="grid gap-5 xl:grid-cols-[20rem_minmax(0,1fr)]"><PartnershipPicker items={list.data?.items ?? []} search={search} selectedId={effectivePartnershipId} loading={list.isLoading} error={list.isError ? errorText(list.error) : undefined} onSearch={setSearch} onSelect={(item) => selectPartnership(item.partnershipId, item.latestTaxYear ?? undefined)} /><section className="min-w-0">{partnership.data ? <TrackerWorkspace detail={detail} partnership={partnership.data} selectedYear={effectiveTaxYear} canEdit={canEdit} onSelectYear={selectYear} onPrefetchYear={prefetchYear} onCompare={() => setComparing(true)} onEdit={() => setEditing(true)} onDelete={() => void deleteYear()} onSignoff={signoff} pending={actions.updateYear.isPending || actions.signoff.isPending} /> : list.isLoading || partnership.isLoading ? <div className="flex min-h-64 items-center justify-center rounded-xl border border-gray-200 bg-white"><Loader2 className="h-6 w-6 animate-spin text-gray-400" /></div> : <div className="rounded-xl border border-dashed border-gray-300 bg-white p-12 text-center text-sm text-gray-500">Choose a partnership to open its tracker.</div>}</section></div>{addingYear && <AddYearDialog defaultTaxYear={defaultTaxYear} pending={actions.createYear.isPending} onClose={() => setAddingYear(false)} onAdd={addYear} />}{editing && detail && <EditYearDrawer detail={detail} pending={actions.calculate.isPending || actions.updateYear.isPending} onClose={() => setEditing(false)} onCalculate={calculate} onSave={save} />}{importing && effectivePartnershipId && <ImportWorkbookDialog partnershipId={effectivePartnershipId} onClose={() => setImporting(false)} onCompleted={(summary) => { setMessage(summary); setImporting(false) }} />}{comparing && partnership.data && <CompareYearsDrawer years={partnership.data.years} selectedYear={effectiveTaxYear} onClose={() => setComparing(false)} />}</>
}
