...
const source = (detail: K1TrackerYearDetail, keys: string | string[]) => {
  ...
  const value = detail.values.find((item) => fieldKeys.includes(item.fieldKey) && item.amount != null)
...
export function ReconciliationPanel({ calculation, detail }: { calculation: K1TrackerCalculation; detail: K1TrackerYearDetail }) {
  const section: Array<[string, unknown, string | null]> = [
    ...
    ['Capital contributions', calculation.sectionL.reportedContributions, 'capital_contributions'],
