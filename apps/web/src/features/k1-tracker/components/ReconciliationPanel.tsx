...
const source = (detail: K1TrackerYearDetail, key: string) => { const value = detail.values.find((item) => item.fieldKey === key && item.amount != null); return value ? `${value.sourceType.replaceAll('_', ' ')}${value.sourceSheet ? ` - ${value.sourceSheet}${value.sourceCell ? `!${value.sourceCell}` : ''}` : ''}` : 'Calculated or not entered' }

export function ReconciliationPanel({ calculation, detail }: { calculation: K1TrackerCalculation; detail: K1TrackerYearDetail }) {
  const section: Array<[string, unknown, string | null]> = [
    ['Reported beginning', calculation.sectionL.reportedBeginning, 'section_l_beginning_capital'],
    ['Reported contributions', calculation.sectionL.reportedContributions, 'section_l_capital_contributed'],
    ['Calculated net income', calculation.sectionL.calculatedNetIncome, null],
