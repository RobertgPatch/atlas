import type { K1TrackerCheckResult, K1TrackerYearDetail } from '../../../../../../packages/types/src/k1-tracker'
...
export const blockingChecksPass = (checks: K1TrackerCheckResult[]): boolean =>
  checks.every((check) => !check.blocking || check.status === 'PASS')
...
export function K1YearResults({ detail, canEdit, pending, onSignoff }: {
  ...
  const checksPassing = blockingChecksPass(detail.calculation.checks)
