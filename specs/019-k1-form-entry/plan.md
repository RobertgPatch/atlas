...
**Scale/Scope**: One partnership workspace tab, one selected tax year, about 42 editable fields, reference-only official cells for lines Jackson does not track, and focused K-1 workflow/accessibility tests  
...
3. **No contract or storage drift**: PASS. The design adds presentation metadata only; no shared wire contract, API route, database table, or migration changes.
...
8. **Truthful representation**: PASS. Unsupported K-1 cells are noninteractive and marked as not tracked; the UI is identified as Jackson data entry rather than an official filed form.
...
- `K1FormReferenceCell` represents an official but unsupported line and is always static, nonfocusable, and excluded from amounts and change sets.
...
- No API or type-package contract is added. The feature contract is the UI mapping and invariant document in `contracts/k1-form-entry-ui.md`.
