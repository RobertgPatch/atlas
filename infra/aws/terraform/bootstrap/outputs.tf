output "state_bucket_name" {
  description = "S3 bucket used by the Atlas Terraform backend."
  value       = aws_s3_bucket.terraform_state.id
}

output "state_bucket_arn" {
  description = "ARN of the Atlas Terraform state bucket."
...
output "backend_config_example" {
  description = "Non-secret partial backend settings. Replace <environment> with staging or production."
  value = {
    bucket       = aws_s3_bucket.terraform_state.id
    key          = "${local.normalized_state_key_prefix}<environment>/terraform.tfstate"
...
output "state_migration_command_example" {
  ...
  value = join(" ", [
    ...
    "-backend-config=\"key=${local.normalized_state_key_prefix}<environment>/terraform.tfstate\"",
